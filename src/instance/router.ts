  ...
  return new Promise((resolve) => {
      ...
      messages: [
        {
          role: 'system',
          content:
            '只回复窗口名。比较用户消息与每个窗口的 labels/task/project/context/title，选最匹配的。',
        },
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            resolve(JSON.parse(d).choices?.[0]?.message?.content?.trim() || null);
          } catch {
            resolve(null);
          }
        });
...
export class Router {
    ...
    if (/^(切换|切到|导航到)/.test(text)) {
      ...
      for (const i of this.reg.list()) {
        ...
        if (
          ...
          (i.name.includes(kw) ||
            i.title.includes(kw) ||
            s.labels.some((l) => l.includes(kw)) ||
            s.task.includes(kw))
        ) {
          this.isCmd = true;
          this.target = i.name;
          return { inst: i, reason: `切→${i.name}` };
        }
      }
    }
    if (/^(新建|创建)/.test(text)) {
      this.isCmd = true;
      const i = this.reg.create();
      return { inst: i, reason: '新建' };
    }
      ...
      messages: [
        {
          role: 'system',
          content:
            '分析消息，更新窗口Schema。返回JSON: {"labels":["新增标签"],"task":"更新后的任务(可选)"}。基于现有信息增量更新，不要覆盖已有标签。只返回JSON。',
        },
        {
          role: 'user',
          content: `当前: labels=[${s.labels.join(',')}] task="${s.task}"\n消息: "${text}"\n更新: 提取新标签，如消息体现任务变化则更新task。`,
        },
      ],
      temperature: 0,
      max_tokens: 80,
    });
    const req = (require('https') as typeof import('https')).request(
      'https://api.deepseek.com/v1/chat/completions',
      {
        method: 'POST',
        timeout: 4000,
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (j.labels)
              this.reg.setSchema(winName, {
                labels: [...new Set([...s.labels, ...j.labels])],
                task: j.task || s.task,
              });
          } catch {
            /* ignore parse errors */
  ...
  status() {
    return `${this.target} (${this.reg.list().length})`;
  }
}
