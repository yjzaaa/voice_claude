...
function L(s: string) {
  if (LOGF)
    try {
      require('fs').appendFileSync(LOGF, s + '\n');
    } catch {}
}
...
class WsReader {
  ...
  attach(sock: net.Socket): void {
    sock.on('data', this.onData);
  }
  detach(sock: net.Socket): void {
    sock.removeListener('data', this.onData);
    this.cancel();
  }
  ...
  private extractFrame(): Buffer | null | undefined {
    ...
    if (len === 126) {
      ...
      if (this.buf.length < 10) return undefined;
...
function parseResp(raw: Buffer): { msgType: number; seq: number; json?: any; raw?: Buffer } | null {
  ...
  if (raw.length >= 12) {
    ...
    if (sz > 0 && sz <= raw.length - 12 && sz < 0x100000)
      candidates.push({ seq: raw.readUInt32BE(4), payload: raw.subarray(12, 12 + sz) });
  }

  // B) v2 风格无 seq: [4B hdr][4B payloadSize][payload]
  if (raw.length >= 8) {
    const sz = raw.readUInt32BE(4);
    if (sz > 0 && sz <= raw.length - 8 && sz < 0x100000)
      candidates.push({ seq: 0, payload: raw.subarray(8, 8 + sz) });
  }
  ...
  if (raw.length > 2) {
    try {
      return { msgType: 0, seq: 0, json: JSON.parse(raw.toString('utf-8')) };
    } catch {}
  }
...
export async function transcribe(audio: Buffer, sampleRate = 16000): Promise<string | null> {
  ...
  try {
    ...
    // 读取 HTTP 响应
    const resp = await readHttpResponse(sock, 5000);
    ...
    if (!upgResp || !upgResp.includes('101')) {
      L('3.FAIL ' + (upgResp ? upgResp.slice(0, 100) : 'timeout'));
      tlsSock.end();
      return null;
    }
    L('3.OK');
    ...
    while (Date.now() < deadline) {
      ...
      if (!raw) {
        L('RESP timeout/close');
        break;
      }
      L(
        `RESP ${raw.length}B hex=${raw.slice(0, 20).toString('hex')} first=${raw.slice(0, 80).toString().replace(/\n/g, '\\n')}`,
      );
